...
class CategoryPromotionTest {
	...
	@BeforeEach
	public void setUp() {
		ReflectionTestUtils.setField(categoryPromotion, "discountPercentage", 0.05);
		ReflectionTestUtils.setField(categoryPromotion, "promotionCategoryId", 3003);
	}

	@Test
	void test_checkPromotion_should_return_true_when_cart_has_category_item(){
		//given
		var cart = new Cart();
		cart.getItems().add(new DefaultItem(1,3003,1,1,1));
		cart.getItems().add(new DefaultItem(2,1,1,1,1));

		//when
		var response = categoryPromotion.checkPromotion(cart);
	...
	@Test
	void test_checkPromotion_should_return_false_when_cart_has_not_seller_item(){
		//given
		var cart = new Cart();
	...
	void test_applyPromotion_should_return_as_expected(){
		...
		var promotion = PromotionDTO.builder().totalDiscount(50).build();

		//when
		var response = categoryPromotion.applyPromotion(cart);
